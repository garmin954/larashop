...
{
    public function setting(Request $request,Store $store_model,Area $area_model){
        $user_info = auth()->user();
        ...
        $area_list = $area_model->whereIn('area_id',$request->area_info)->get();
        $data = [
    ...
    // 获取店铺申请的分类
    public function get_store_class(Request $request,StoreClass $store_class_model,Store $store_model,GoodsClass $goods_class_model){
        $user_info = auth()->user();
        $store_info = $store_model->where('user_id',$user_info['id'])->first();
        ...
        $deep = $request->deep; // 深度
        if($deep==0){
            $arr = explode(',',$class_info['class_2']);
            $arr = array_unique($arr);
            $list = $goods_class_model->whereIn('id',$arr)->where('pid',$request->id)->get();
        }
        if($deep==1){
    ...
    // 设置包邮，多少钱免邮费
    public function free_freight(Request $request,Store $store_model){
        $user_info = auth()->user();
        ...
        $free_freight = $request->free_freight;
        $rs = $store_model->where('id',$store_info['id'])->update(['free_freight'=>$free_freight]);
        return $this->success_msg('ok',$rs);
    }

    // 设置商品详情 售后服务展示图片
    public function after_sale_content(Request $request,Store $store_model){
        $user_info = auth()->user();
        ...
        $after_sale_content = $request->after_sale_content;
