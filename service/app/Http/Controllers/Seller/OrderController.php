...
{
    public function index(Request $request,Order $order_model){
    ...
    public function info(Request $request,Order $order_model){
        $id = $request->id;
        $user_info = auth()->user();
    ...
    // 发货物流填写
    public function send_delivery(Request $request,Order $order_model){
    ...
    // 同意退款
    public function refund_money(Request $request,Order $order_model,Users $user_model){
        $id = $request->id;
        $user_info = auth()->user();
        $buyer_info = $user_model->find($request->user_id);
        ...
        $user_model->money_change('售后退款','frozen_money',-$order_info['total_price'],$buyer_info,'order_no:'.$request->order_no,'-');
        $user_model->money_change('售后退款','money',$order_info['total_price'],$buyer_info,'order_no:'.$request->order_no,'-');
        return $this->success_msg('成功');
    }
}
