...
class GoodsController extends BaseController
{
    ...
    // 添加商品
    public function add(Request $request,Goods $goods_model,GoodsBrand $goods_brand_model,GoodsSpec $goods_spec_model,Config $config_model,GoodsClass $goods_class_model,StoreGoodsClass $store_goods_class_model){
        ...
        $chose_spec_id = '';
        if(isset($request->chose_spec)){                                            // 选择的规格 非使用属性
            foreach($request->chose_spec as $v){
                if($chose_spec_id == ''){
                    $chose_spec_id = $v['id'];
                }else{
                    $chose_spec_id .= ','.$v['id'];
        ...
        $chose_attr = '';                                                       // 选择属性
        if(!empty($request->chose_attr)){
            $chose_attr = implode(',',$request->chose_attr);
        }

        $postData = [
            'user_id'               =>  intval($user_info['id']),               // 用户ID
            'cid'                   =>  intval($request->cid),                      // 分类ID
            'bid'                   =>  intval($request->bid),                      // 品牌ID
            'goods_no'              =>  $request->goods_no??'',                     // 商品编号
            'goods_name'            =>  $request->goods_name,                       // 商品名
            'goods_subname'         =>  $request->goods_subname??'',                // 商品副标题
            ...
            'goods_freight'         =>  abs(floatval($request->goods_freight)),     // 商品运费
            'freight_id'            =>  $request->freight_id??0,                    // 商品运费模版
            'chose_spec_id'         =>  $chose_spec_id,                         // 选择的规格 (为了方便编辑存储)
            'chose_attr'            =>  $chose_attr,                            // 选择的属性 (为了方便编辑存储)
            'is_groupbuy'           =>  $request->is_groupbuy??0,                   // 是否参加拼团
            'groupbuy_price'        =>  $request->groupbuy_price??0,                // 拼团商品价格
            'groupbuy_num'          =>  $request->groupbuy_num??0,                  // 拼团需要人数
            ...
            'store_goods_class'     =>  $request->store_goods_class??0,             // 自定义分类
            'is_top'                =>  $request->is_top,                           // 是否置顶
            'add_time'              =>  time(),
            'edit_time'             =>  time(),
        ];
        DB::beginTransaction();
        try{
            ...
            // 循环插入属性
            if(isset($request->spec_attr)){
                $spec_data = [];
                foreach($request->spec_attr as $v){
                    $spec_info = [];
    ...
    // 编辑商品
    public function edit(Request $request,Goods $goods_model,GoodsBrand $goods_brand_model,GoodsSpec $goods_spec_model,Config $config_model,AttrSpec $attr_spec_model,GoodsClass $goods_class_model,FreightTemplate $freight_template_model,StoreGoodsClass $store_goods_class_model,$id){
        if(!$request->isMethod('post')){
            $data['goods_brand_list'] = $goods_brand_model->get();
            $data['store_goods_class'] = $store_goods_class_model->get();
        ...
        $postData = [
            ...
            'goods_status'          =>  $request->goods_status,                                 // 商品状态 上架 下架
            'goods_verify'          =>  $goods_verify,                                      // 商品审核状态 1 通过 0 审核失败 2 审核中
            'goods_content'         =>  empty($request->goods_content)?'':$request->goods_content,  // 商品内容
            'store_goods_class'     =>  $request->store_goods_class??0,                         // 自定义分类
            'is_top'                =>  $request->is_top,                                       // 是否置顶
            'edit_time'             =>  time(),
        ];
    ...
    // 删除商品
    public function del(Request $request,Goods $goods_model,GoodsSpec $goods_spec_model){
        $id = $request->id;
        $user_info = auth()->user();
        $ids = explode(',',$id);
    ...
    // 指定上架
    public function goods_status(Request $request,Goods $goods_model){
        $id = $request->id;
        $user_info = auth()->user();
