...
class StoreController extends BaseController
{
    public function index(Request $request,Store $store_model){
        ...
        if(isset($request->store_verify)){
            $where['store_verify'] = $request->store_verify;
        }
	...
	public function del(Request $request,Store $store_model){
		$id = $request->id;
		$ids = explode(',',$id);
    ...
    // 修改店铺信息  审核之类的
    public function store_pass(Request $request,Store $store_model,Roles $roles_model,Users $users_model){
        $info = $request->only(['store_verify','store_status','store_close_info','id']);
