...
class RolesController extends BaseController
{
    ...
    // 添加
    public function add(Request $request,Roles $roles_model,Menus $menus_model,Hooks $hooks_model){
		...
		try{
			...
			$roles = $roles_model->create(['name'=>$request->name,'content'=>$request->content]);

			// 菜单
			if(!empty($request->menus)){
				// $menus_info = [];
				// $menus_permission = [];
				// foreach($request->menus as $v){
				// 	$arr = [];
				// 	$arr = explode(',',$v);
				...
				// }
				$roles->menus()->attach($request->menus);
			}

			// 钩子
			if(!empty($request->hooks)){
				$roles->hooks()->attach($request->hooks);
			}
    ...
    // 编辑
    public function edit(Request $request,Roles $roles_model,Menus $menus_model,Hooks $hooks_model,$id){
		...
		try{
			...
			$roles = $roles_model->where('id',$id)->update(['name'=>$request->name,'content'=>$request->content]);
			...
			if(!empty($request->menus)){
				...
				// 	$menus_permission[$k] = $v;
				// }
				$roles_info->menus()->sync($request->menus);
			}else{
			...
			// 钩子
			if(!empty($request->hooks)){
				$roles_info->hooks()->sync($request->hooks);
			}else{
    ...
    // 删除
    public function del(Request $request,Roles $roles_model){
