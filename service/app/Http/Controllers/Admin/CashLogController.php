...
{
    public function index(Request $request,CashLog $cash_log_model){
        $list = $cash_log_model->orderBy('status','desc')->orderBy('id','desc')->paginate(20);
        return $this->success_msg('ok',$list);
    }

    public function change_status(Request $request,CashLog $cash_log_model){
        $id = $request->id;
    ...
    public function del(Request $request,CashLog $cash_log_model){
        $id = $request->id;
        $ids = explode(',',$id);
