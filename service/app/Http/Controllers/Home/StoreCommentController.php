...
class StoreCommentController extends BaseController
{
    ...
    // 根据商品ID 获取评论列表
    public function get_comment_list_by_goods(Request $request,StoreComment $store_comment_model){
        ...
        $store_comment_model = $store_comment_model->where('goods_id',$request->goods_id);
        if($request->comment_type == 1){
            $store_comment_model = $store_comment_model->where('score','>=',5);
        }elseif($request->comment_type == 2){
            $store_comment_model = $store_comment_model->where('score','<',5)->where('score','>=',3);
        }elseif($request->comment_type == 3){
            $store_comment_model = $store_comment_model->where('score','<',3);
        }

        $data['all'] = $store_comment_model3->where('goods_id',$request->goods_id)->count();
        $data['good'] = $store_comment_model3->where('goods_id',$request->goods_id)->where('score','>=',5)->count();
        $data['secondary'] = $store_comment_model4->where('goods_id',$request->goods_id)->where('score','<',5)->where('score','>=',3)->count();
        $data['bad'] = $store_comment_model5->where('goods_id',$request->goods_id)->where('score','<',3)->count();

        $data['comment_list'] =  $store_comment_model->paginate(20)->toArray();
        $num = $store_comment_model2->where('goods_id',$request->goods_id)->sum('score');
    ...
    public function add_comment(Request $request,StoreComment $store_comment_model,Order $order_model){
        $user_info = auth()->user();
        $goods_list = $request->goods_list;
        $comment_image = '';

        if(!empty($request->comment_list)){
            foreach($request->comment_list as $v){
        ...
        $order_info = $order_model->where('id',$request->order_id)->first();
        if($order_info['comment_status'] == 1){
            return $this->success_msg('已经评论过了');
        }else{
            $order_model->where('id',$request->order_id)->update(['comment_status'=>1,'comment_time'=>time()]);
        }

        foreach($goods_list as $v){
            $data['content'] = $request->content;
            $data['score'] = round(($request->agree+$request->service+$request->speed)/3/5,1);
