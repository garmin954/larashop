...
class IntegralController extends NotokenController
{
    ...
    // 获取搜索商品
    public function search_integral_goods(Request $request,IntegralGoods $integral_goods_model){
        $where = [];
        $keywords = '';

        // 分类判断
        if(!empty($request->goods_class_id) && isset($request->goods_class_id)){
            $where['cid'] = $request->goods_class_id;
        }
