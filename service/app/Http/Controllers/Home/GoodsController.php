...
{
    // 商品详情
    public function get_goods_info(Request $request,Goods $goods_model,GoodsClass $goods_class_model,Store $store_model,StoreComment $store_comment_model,Groupbuy $groupbuy_model,GroupbuyUser $groupbuy_user_model){
    ...
    public function get_sale_list(Request $request,Goods $goods_model){
    ...
    // 获取搜索商品
    public function search_goods(Request $request,Goods $goods_model,GoodsClass $goods_class_model){
        $where = [];
        $keywords = '';

        // 品牌判断
        if(!empty($request->goods_brand) && isset($request->goods_brand)){
            $where['bid'] = $request->goods_brand;
        }

        // 团购产品
        if(!empty($request->is_groupbuy) && isset($request->is_groupbuy)){
            $where['is_groupbuy'] = 1;
        }
        ...
        if(isset($request->goods_type) && isset($request->order_type)){
            ...
            $order['order_type'] = empty($request->order_type)?'desc':'asc';
        }
        ...
        $class_id = 0;
        if(isset($request->class_info) && count($request->class_info)>0){
            $class_id = $request->class_info[count($request->class_info)-1];
        }
